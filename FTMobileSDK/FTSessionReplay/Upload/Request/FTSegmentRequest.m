...
- (void)requestWithEvent:(NSData *)event parameters:(NSDictionary *)parameters{
    self.parameters = parameters;
    FTSegmentJSON *segment = [[FTSegmentJSON alloc]initWithData:event];
...
- (NSMutableURLRequest *)adaptedRequest:(NSMutableURLRequest *)mutableRequest{
    ...
    if(self.multipartFormBody){
        if(self.segments && self.segments.count>0){
            ...
            [self.multipartFormBody addFormField:@"compressed_segment_size" value:[NSString stringWithFormat:@"%ld",compress.length]];
            mutableRequest.HTTPBody = [self.multipartFormBody build];
        }
    }
