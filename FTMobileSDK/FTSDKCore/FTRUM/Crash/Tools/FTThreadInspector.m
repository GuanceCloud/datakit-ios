...
typedef struct {
    ...
    FTStackEntry stackEntries[MAX_STACKTRACE_LENGTH];
    int stackLength;
} FTThreadInfo;

unsigned int
getStackEntriesFromThread(FTCrashThread thread, struct FTCrashMachineContext *context,
                          FTStackEntry *buffer, unsigned int maxEntries, bool symbolicate)
{
    ftcrashmc_getContextForThread(thread, context, NO);
    FTStackCursor stackCursor;

    ftsc_initWithMachineContext(&stackCursor, MAX_STACKTRACE_LENGTH, context);
...
- (NSArray<FTThread *> *)getCurrentThreadsWithStackTrace{
    ...
    @synchronized(self) {
        ...
        ftmc_resumeEnvironment(suspendedThreads, numSuspendedThreads);
