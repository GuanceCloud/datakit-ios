...
#ifndef FTDL_MaxCrashInfoStringLength
#define FTDL_MaxCrashInfoStringLength 4096
#endif
...
#pragma pack()
#define FTDL_SECT_CRASH_INFO "__crash_info"
...
{
    if (imageName != NULL) {
        const uint32_t iImg = ftdl_imageNamed(imageName, exactMatch);
        if (iImg != UINT32_MAX) {
...
static bool isValidCrashInfoMessage(const char *str)
{
    ...
    int maxReadableBytes = ftmem_maxReadableBytes(str, FTDL_MaxCrashInfoStringLength + 1);
...
static void getCrashInfo(const struct mach_header *header, FTBinaryImage *buffer)
{
    unsigned long size = 0;
    crash_info_t *crashInfo =
        (crash_info_t *)getsectiondata((mach_header_t *)header, SEG_DATA, FTDL_SECT_CRASH_INFO, &size);
    if (crashInfo == NULL) {
        return;
    }

    //("Found crash info section in binary: %s", buffer->name);
    const unsigned int minimalSize = offsetof(crash_info_t, reserved);  // Include message and message2
    if (size < minimalSize) {
        //("Skipped reading crash info: section is too small");
        return;
    }
    if (!ftmem_isMemoryReadable(crashInfo, minimalSize)) {
...
bool ftdl_getBinaryImage(int index, FTBinaryImage *buffer)
{
    ...
    return ftdl_getBinaryImageForHeader((const void *)header, _dyld_get_image_name((unsigned)index), buffer,false);
}
