...
#ifndef FTCrashJailbreak_h
#define FTCrashJailbreak_h
...
static inline bool ftcrashj_local_is_insert_libraries_env_var(const char *str)
{
    if (str == NULL) {
        return false;
    }
...
#define FTCRASH_HAS_CUSTOM_SYSCALL 1
...
#define ftcrashj_syscall3(call_num, param0, param1, param2, pResult)     \
    do {                                                            \
...
#else
#define FTCRASH_HAS_CUSTOM_SYSCALL 0
...
#if FTCRASH_HAS_CUSTOM_SYSCALL

// See: https://opensource.apple.com/source/xnu/xnu-7195.81.3/bsd/kern/syscalls.master
#define FTCRASH_SYSCALL_OPEN 5
#define ftcrashj_syscall_open(path, flags, mode, pResult) \
    ftcrashj_syscall3(FTCRASH_SYSCALL_OPEN, (uintptr_t)path, flags, mode, pResult)
...
#endif /* FTCRASH_HAS_CUSTOM_SYSCALL */
 ...
 */
#if !TARGET_OS_SIMULATOR && !TARGET_OS_OSX && FTCRASH_HAS_SYSCALL
#define get_jailbreak_status(pIsJailbroken)                                                       \
    do {                                                                                          \
        ...
        ftcrashj_syscall_open(test_write_file, O_CREAT, 0644, &fd);                                    \
        if (fd > 0) {                                                                             \
            close(fd);                                                                            \
            tmp_file_is_accessible = true;                                                        \
        } else {                                                                                  \
            ftcrashj_syscall_open(test_write_file, O_RDONLY, 0, &fd);                                  \
            if (fd > 0) {                                                                         \
                close(fd);                                                                        \
                tmp_file_is_accessible = true;                                                    \
            }                                                                                     \
        ...
        ftcrashj_syscall_open(mobile_substrate_path, O_RDONLY, 0, &fd);                                \
        if (fd > 0) {                                                                             \
            close(fd);                                                                            \
        ...
        for (int i = 0; environ[i] != NULL; i++) {                                                \
            if (ftcrashj_local_is_insert_libraries_env_var(environ[i])) {                              \
                has_insert_libraries = true;                                                      \
                break;                                                                            \
            }                                                                                     \
        }                                                                                         \
                                                                                                  \
        const char *sandboxpath = "/private/ftcrash_jailbreak_test";                              \
        ftcrashj_syscall_open(sandboxpath, O_WRONLY | O_CREAT | O_TRUNC, 0644, &fd);                   \
        if (fd > 0) {                                                                             \
            close(fd);                                                                            \
            broken_sandbox = true;                                                                \
            remove(sandboxpath);                                                                  \
