...
#ifndef FTCrashObjCApple_h
#define FTCrashObjCApple_h
...
#if OBJC_HAVE_TAGGED_POINTERS // (FTCrash) This line is moved here to make `objc_tag_index_t` enum visible for i386.
...
// (FTCrash) Added to handle runtime checks for Split Tagged Pointers,
...
static inline bool
ftcrashc_objc_splitTaggedPointersEnabled(void);
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 8 : 16;
}

static inline uintptr_t
X_OBJC_TAG_SLOT_MASK_func(void)
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 0x7UL : 0xfUL;
}

static inline uintptr_t
X_OBJC_TAG_MASK_func(void)
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? (1UL<<63) :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 0 :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 1 :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 4 :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? (X_OBJC_TAG_MASK_func() | 0x7UL) :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? ((1UL<<62) | X_OBJC_TAG_EXT_MASK_func()) : 0;
}

static inline int
X_OBJC_TAG_EXT_SLOT_SHIFT_func(void)
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 55 :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ?
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 9 :
...
{
    return ftcrashc_objc_splitTaggedPointersEnabled() ? 12 :
        OBJC_MSB_TAGGED_POINTERS ? 12 : 12;
}

// (FTCrash) Macros were rewritten to include runtime checks for split tagged pointers, based on objc4-912.3.
...
// (FTCrash) Weakly linked to support *OS versions prior to objc4-818.2, where this symbol may not be available.
...
static inline uintptr_t _objc_basicTagToObfuscatedTag(uintptr_t tag) {
    ...
    // (FTCrash) Fallback: return the original tag if permutations are unavailable or tag is out of range.
...
static inline uintptr_t _objc_obfuscatedTagToBasicTag(uintptr_t tag) {
    ...
    // (FTCrash) Fallback: return the original tag if within range, otherwise return 7.
    // Runtime handles the availability of split tagged pointers, so fallback is typically unnecessary,
    // but it's included as a safeguard.
    return (tag < 8) ? tag : 7;
}
...
#if OBJC_SPLIT_TAGGED_POINTERS
    if (ftcrashc_objc_splitTaggedPointersEnabled()) {
